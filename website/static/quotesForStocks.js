async function getquotes(symbol) {
  return new Promise((resolve, reject) => {
    ...
    var url =
      ...
      "&to=" +
      to +
      "&countback=2000000&currencyCode=INR";
    //check
...
export function chartdisplay(symbol) {
  ...
  anychart.onDocumentReady(async function () {
    ...
    var date = new Date();
    var day = date.getTime() - 100 * 24 * 60 * 60 * 1000;
    date.setTime(day);
    var from = date.toLocaleDateString("en-US");
    //  var from=(today.getMonth())+'/'+today.getDate()+'/'+today.getFullYear()-1;
